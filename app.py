...
Project: Customer Segmentation with Streamlit & KMeans
"""
...
def load_data(uploaded_file):
    if uploaded_file is not None:
        ...
        df = pd.read_csv(uploaded_file, encoding='latin-1', sep=r'\s+', header=None,
...
if choice == 'Business Understanding':
    ...
    st.write("""
    ###### Customer segmentation is a fundamental task in marketing and customer relationship management. 
    With the advancements in data analytics and machine learning, it is now possible to group customers into 
    distinct segments with a high degree of precision, allowing businesses to tailor their marketing strategies 
    and offerings to each segment's unique needs and preferences.

    ###### Through customer segmentation, businesses can achieve:
    ...
    """)
    st.image("Customer.png", caption="Customer Segmentation", use_column_width=True)

elif choice == 'Data Understanding':    
    # List all files in 'data' folder
    sample_files = os.listdir('data')
    
    # Radio button to choose sample file or upload
    ...
    if st.session_state['df'] is not None:
        ...
        # Options for handling missing and duplicate values
        ...
        st.write("Transactions timeframe from {} to {}".format(
            st.session_state['df']['day'].min(), st.session_state['df']['day'].max()))
        st.write("{:,} transactions without a customer id".format(
            st.session_state['df'][st.session_state['df'].Customer_id.isnull()].shape[0]))
    ...
    if st.session_state['df'] is not None:
        ...
        n_clusters = st.sidebar.number_input('Choose number of clusters k from 2 to 20:', min_value=2, max_value=20, value=3, step=1)
        st.write(f'You have chosen {n_clusters} clusters.')
        ...
        if st.button('Export Model'):
            ...
            st.write('Model (kmeans_model.pkl) exported successfully!')
    ...
    if 'model_exported' in st.session_state and st.session_state.model_exported:
        ...
        recent_date = st.date_input('Most Recent Purchase Date:')
